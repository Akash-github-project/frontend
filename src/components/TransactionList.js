...
const TransactionList = ({ userId = 1 }) => {
	let headers = ["Sr No", "Date", "Activity", "Debit", "Credit"];
						...
						transaction[`${key}`].map(each => (
							<>
								<div className="text-gray-800 flex items-center justify-center text-sm lg:text-base ">
									<span className="text-gray-primary text-sm">{each.sno}</span>
								</div>

								<div className="text-gray-800 flex items-center justify-center text-sm lg:text-base ">
									<span className="text-gray-primary text-sm">{each.date}</span>
								</div>
								<div className="text-gray-800 flex items-center justify-center text-sm lg:text-base ">
